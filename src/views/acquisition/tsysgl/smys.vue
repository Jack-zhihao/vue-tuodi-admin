 ...
 * @Date: 2019-03-19 16:43:08
 * @LastEditTime: 2020-03-06 16:41:30
 -->
...
export default {
  ...
  methods: {
    ...
    calculateTaojia() {
      let count =
        Number(this.infoForm.juance).toFixed(2) || Number(0).toFixed(2);
      let cejia =
        Number(this.infoForm.cejia).toFixed(2) || Number(0).toFixed(2);
      this.infoForm.taojia = (count * cejia).toFixed(2);
    },
    searchBookBySmid() {
      searchBookBySmid({
        ...
        if (!this.infoForm["cejia"]) {
          this.infoForm["cejia"] = this.infoForm["yuanjia"].toFixed(2);
        }
        // 没有分册数量默认1
    ...
    searchBookByMarcid() {
      searchBookByMarcid({
        ...
        // 设置值
        this.infoForm["sshao"] = res.data.sshao;
        this.infoForm["yuanjia"] = res.data.jge.toFixed(2);
        this.infoForm["fuben"] = localStorage.getItem("td-zjysfuben") || "";
        // 册价默认取原价的值
        if (!this.infoForm["cejia"]) {
          this.infoForm["cejia"] = res.data.jge.toFixed(2);
        }
        // 没有分册数量默认1
        if (!this.infoForm["juance"]) {
          this.infoForm["juance"] = 1;
