 ...
 * @LastEditTime: 2020-03-10 09:30:04
 -->

<template>
  <div class="dzgg common__content">
    <td-content-header>
      读者公告
            ...
            <template slot-scope="scope">
              ...
              >
                <template v-slot:menus>
                  <div v-if="scope.row.readerNoticeZtai == '0'">
      ...
      <div class="edit__content">
        ...
        >
          <div class="custom__form_wrapper">
            ...
            <el-form-item></el-form-item>
            <el-form-item class="item" label="公告类型">
              <el-select
                filterable
                v-model="literatureAddForm.typeId"
                placeholder="请选择"
                style="flexBasis:150px"
                @change="changeTypeAdd"
              >
                <el-option
                  v-for="item in literatureAddForm.typeArr"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
            ...
            <el-form-item></el-form-item>
            <el-form-item class="item" prop="sysNoticeTitle" label="通知标题">
              <el-input
                clearable
            ...
            <el-form-item
              class="item"
              label="公告内容"
              prop="sysNoticeContent"
              v-if="this.literatureAddForm.typeId == '1'"
            >
              <editor-bar
        ...
        >
          <div class="custom__form_wrapper">
            ...
            <el-form-item class="item" label="公告类型">
              <el-select
                filterable
                v-model="literatureEditForm.typeId"
                placeholder="请选择"
                style="flexBasis:150px"
                @change="changeTypeEdit"
              >
                <el-option
                  v-for="item in literatureEditForm.typeArr"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                >
            ...
            <el-form-item
              ...
              label="公告内容"
              prop="sysNoticeContent"
              v-if="this.literatureEditForm.typeId == '1'"
            >
              <editor-bar
...
export default {
  ...
  methods: {
    ...
    handleLiteratureFormAdd(index) {
      let params = {
        readerNoticeMainLibId: this.literatureAddForm.sysNoticeMainLibId,
        readerNoticeTitle: this.literatureAddForm.sysNoticeTitle,
        readerNoticeContent: this.literatureAddForm.editor.sysNoticeContent,
        readerNoticeNote: this.literatureAddForm.sysNoticeNote,
        readerNoticeZtai: index,
        affiliateLibs: this.literatureAddForm.affiliateLibs.join(),
        readerNoticeType: this.literatureAddForm.typeId
      };
      saveDzgg(params).then(res => {
        if (res.code == 0) {
          this.$message({
            message: res.message,
            type: "success"
    ...
    handleLiteratureFormEdit(index) {
      let params = {
        readerNoticeMainLibId: this.literatureEditForm.sysNoticeMainLibId,
        readerNoticeTitle: this.literatureEditForm.sysNoticeTitle,
        readerNoticeContent: this.literatureEditForm.editor.sysNoticeContent,
        readerNoticeNote: this.literatureEditForm.sysNoticeNote,
        readerNoticeZtai: index,
        affiliateLibs: this.literatureEditForm.affiliateLibs.join(),
        readerNoticeType: this.literatureEditForm.typeId,
        readerNoticeId: this.literatureEditForm.readerNoticeId
      };
      saveDzgg(params).then(res => {
        if (res.code == 0) {
          this.$message({
    ...
    editGzsm(row) {
      ...
      this.literatureEditForm.sysNoticeMainLibId = row.readerNoticeMainLibId;
      this.literatureEditForm.affiliateLibs = row.affiliateLibs
        ? row.affiliateLibs.split(",")
        : [];
      this.literatureEditForm.sysNoticeTitle = row.readerNoticeTitle;
      this.literatureEditForm.editor.sysNoticeContent = row.readerNoticeContent;
      this.literatureEditForm.sysNoticeNote = row.readerNoticeNote;
      this.literatureEditForm.readerNoticeId = row.readerNoticeId;
      this.literatureEditForm.typeId = row.readerNoticeType;

      if (row.readerNoticeContent) {
        //  消除富文本没填的验证
        this.literatureEditForm.sysNoticeContent = row.readerNoticeContent;
      }
    },
    // 单个删除
    singleDeleteGzsm(row) {
      this.$confirm("确定要删除么?", "提示", {
        ...
        .then(() => {
          delDzgg({ readerNoticeIds: row.readerNoticeId }).then(res => {
            if (res.code == 0) {
              this.$message({
                message: res.message,
                type: "success"
    ...
    deleteGzsm() {
      this.$confirm("确定要删除么?", "提示", {
        ...
        .then(() => {
          delDzgg({
            readerNoticeIds: this.multipleLiteratureSelection.join()
          }).then(res => {
            if (res.code == 0) {
              this.$message({
                message: res.message,
    ...
    changeTypeEdit() {
      if (this.literatureEditForm.typeId == 0) {
        this.literatureEditForm.editor.sysNoticeContent = "";
        this.literatureEditForm.sysNoticeContent = "";
      }
    },
    changeTypeAdd() {
      if (this.literatureAddForm.typeId == 0) {
        this.literatureAddForm.editor.sysNoticeContent = "";
    ...
    upStatus(status, row) {
      dzZtai({
        readerNoticeIds: row.readerNoticeId,
        readerNoticeZtai: status
      }).then(res => {
        if (res.code == 0) {
