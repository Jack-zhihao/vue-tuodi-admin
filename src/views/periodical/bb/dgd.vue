<template>
  <div class="per_dgd common__content">
    <td-content-header>
      <span>订购单</span>
...
export default {
  ...
  methods: {
    ...
    handleDGDSizeChange(pageSize) {
      this.DGDPageSize = pageSize;
      this.currentDGDPage = 1;
      this.getDgd();
    },
    handleDGDCurrentChange(pageNumber) {
      this.currentDGDPage = pageNumber;
      this.getDgd();
    },
    initZDNF() {
    ...
    rowClick(row) {
      this.$refs.DGDList.toggleRowSelection(row);
    }
  },
  mounted() {
    this.initData();
